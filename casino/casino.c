...
void casino()
{
    ...
    while (try --)
    {
        ...
        if (read_int() == 1)
        {
            printf("Which number [1 ~ 6]: ");     // casino+236
            idx = read_int() - 1;                 // index is the GOT address
            printf("Chose the number %d: ", idx); // new instruction address
            guess[idx] = read_int();
        }
...
int main()
{
    ...
    printf("Your name: ");
    read(0, name, 0x100); // potential overflow: shellcode

    printf("Your age: ");
    age = read_int(); // no overflow
